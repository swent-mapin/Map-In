...
class OfflineRegionManager(
  ...
  suspend fun downloadRegion(
    ...
    if (!isConnected) {
      Log.w(TAG, "Device offline, skipping download")
    ...
    try {
      ...
      currentDownload =
          tileStore.loadTileRegion(
              ...
              { progress ->
                ...
                currentDownload = null
                if (expected.isValue) {
                  Log.w(TAG, "Region download completed: $tileRegionId")
      ...
      Log.w(TAG, "Started download for region: $tileRegionId")
    } catch (e: Exception) {
  ...
  fun cancelActiveDownload() {
    currentDownload?.cancel()
    currentDownload = null
    Log.w(TAG, "Active download cancelled")
  }
  ...
  suspend fun removeTileRegion(bounds: CoordinateBounds): Result<Unit> {
    return try {
      ...
      Log.w(TAG, "Removed tile region: $tileRegionId")
      Result.success(Unit)
    } catch (e: Exception) {
