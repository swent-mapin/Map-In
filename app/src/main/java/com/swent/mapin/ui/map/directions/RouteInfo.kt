 ...
 */
data class RouteInfo(val distance: Double, val duration: Double) {
  companion object {
    val ZERO = RouteInfo(distance = 0.0, duration = 0.0)
  }

  /** Formats distance for display. Returns "X km" for distances >= 1000m, "X m" otherwise. */
  ...
  /**
   * Formats duration for display. Returns "X h Y min" for durations >= 1 hour, "X min" otherwise.
   */
