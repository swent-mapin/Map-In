...
class EventBasedOfflineRegionManager(
        ...
        scope.launch {
          ...
          combine(onSavedEventsFlow, onJoinedEventsFlow) { saved, joined -> saved + joined }
              .collect { allEvents ->
                ...
                Log.w(TAG, "Events updated: ${uniqueEvents.size} unique events to process")

                // Only download if online
                if (connectivityService.isConnected()) {
                  downloadRegionsForEvents(uniqueEvents)
                } else {
                  Log.w(TAG, "Device offline, skipping region downloads")
                }
              }
        }
  }
  ...
  suspend fun downloadRegionsForEvents(events: List<Event>) {
    ...
    for (event in eventsToDownload) {
      ...
      if (downloadedEventIds.contains(event.uid)) {
        Log.w(TAG, "Event ${event.uid} already downloaded, skipping")
        continue
      }
      ...
      result
          .onSuccess {
            ...
            Log.w(TAG, "Successfully downloaded region for event: ${event.title}")
  ...
  private suspend fun downloadRegionSuspend(event: Event, bounds: CoordinateBounds): Result<Unit> {
    ...
    offlineRegionManager.downloadRegion(
        bounds = bounds,
        onProgress = { progress ->
          Log.w(TAG, "Event ${event.uid} download progress: ${(progress * 100).toInt()}%")
    ...
    scope.launch {
      for (eventId in eventIds) {
        ...
        if (location != null) {
          ...
          Log.w(TAG, "Deleting region for removed event: $eventId")
          ...
          result
              .onSuccess {
                ...
                Log.w(TAG, "Successfully deleted region for event: $eventId")
              }
              .onFailure { error ->
          ...
          downloadedEventIds.remove(eventId)
          Log.w(TAG, "Removed event $eventId from tracking (no location stored)")
        }
      }
    }
  }
  ...
  fun deleteRegionForEvent(event: Event) {
    scope.launch {
      ...
      Log.w(TAG, "Deleting region for event: ${event.title}")
      ...
      result
          .onSuccess {
            ...
            Log.w(TAG, "Successfully deleted region for event: ${event.title}")
          }
          .onFailure { error ->
  ...
  fun stopObserving() {
    ...
    Log.w(TAG, "Stopped observing events")
  }
  ...
  fun clearDownloadedEventIds() {
    ...
    Log.w(TAG, "Cleared downloaded event IDs and locations")
  }
