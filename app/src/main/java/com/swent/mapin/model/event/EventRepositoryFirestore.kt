...
class EventRepositoryFirestore(private val db: FirebaseFirestore) : EventRepository {
    ...
    override fun getNewUid(): String =
        db.collection(EVENTS_COLLECTION_PATH).document().id
    ...
    override suspend fun getEventsByTags(tags: List<String>): List<Event> {
        ...
        val limited = if (tags.size > 10) tags.take(10) else tags
        val snap = db.collection(EVENTS_COLLECTION_PATH)
    ...
    private fun documentToEvent(document: DocumentSnapshot): Event? = try {
