 ...
 *
 * This implementation bypasses Firebase Cloud Functions and makes HTTP calls directly
 * to the OpenRouter API using Amazon Nova 2 Lite model.
 *
...
class DirectOpenRouterRepository(
  ...
  override suspend fun recommendEvents(
    ...
    try {
      ...
      val eventsFormatted = request.events.map { event ->
        val startDate = event.startTime?.let {
        ...
        mapOf(
            ...
            "distance" to if (event.distanceKm != null) "${String.format(Locale.ENGLISH, "%.1f", event.distanceKm)} km" else "Distance unknown",
            "price" to if (event.price == 0.0) "Free" else "${event.price} CHF",
            "places" to if (event.capacityRemaining != null) "${event.capacityRemaining} places remaining" else "Unlimited capacity"
        )
      }
...
Recommend 2-3 relevant events with selling reasons.
      ...
      val cleanedResponse = aiResponseText
          .replace("```json\n", "")
          .replace("```json", "")
          .replace("```\n", "")
          .replace("```", "")
      ...
      val aiResponse = try {
        ...
        Log.d(TAG, "Successfully parsed ${parsed.recommendedEvents.size} recommended events")
        parsed
      } catch (e: Exception) {
        Log.e(TAG, "Failed to parse AI response: ${e.message}")

        AiRecommendationResponse(
            assistantMessage = "Sorry, I couldn't properly analyze the available events. Can you rephrase your request?",
