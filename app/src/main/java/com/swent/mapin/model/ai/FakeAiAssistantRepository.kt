 ...
 *
 * This implementation provides deterministic responses without making network calls, allowing UI
 * development and testing to proceed independently of the backend.
 */
  ...
  override suspend fun recommendEvents(
    ...
    if (request.events.isEmpty()) {
      return AiRecommendationResponse(
          assistantMessage =
              "I couldn't find any events matching your criteria. Try adjusting your search filters or location.",
    ...
    val recommendedEvents =
        eventsToRecommend.mapIndexed { index, event ->
          AiRecommendedEvent(
              id = event.id,
              reason =
                  when (index) {
    ...
    val assistantMessage =
        when (eventsToRecommend.size) {
          1 ->
              "Based on your query, I recommend checking out $eventTitles. It looks like a great match for what you're looking for!"
          else ->
              "I found ${eventsToRecommend.size} events that might interest you: $eventTitles. Both look exciting!"
        }
