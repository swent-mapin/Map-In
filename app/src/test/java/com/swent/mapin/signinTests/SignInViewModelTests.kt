 ...
 *
 * Tests the authentication logic and UI state management using Robolectric
 * to mock Android framework dependencies like Firebase.
 */
...
class SignInViewModelTest {
    ...
    @Test
    fun `signInWithGoogle should prevent concurrent sign-in attempts when already loading`() = runTest {
    ...
    fun `SignInUiState should support all property combinations`() {
        ...
        val state1 = SignInUiState(isLoading = true, errorMessage = null, isSignInSuccessful = false, currentUser = null)
        assertTrue(state1.isLoading)
        assertNull(state1.errorMessage)

        val state2 = SignInUiState(isLoading = false, errorMessage = "Error", isSignInSuccessful = false, currentUser = null)
        assertFalse(state2.isLoading)
        assertEquals("Error", state2.errorMessage)

        val state3 = SignInUiState(isLoading = false, errorMessage = null, isSignInSuccessful = true, currentUser = mockUser)
