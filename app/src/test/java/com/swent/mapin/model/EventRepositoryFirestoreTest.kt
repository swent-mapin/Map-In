...
/**
 * Tests for [EventRepositoryFirestore].
 * Uses Mockito to mock Firestore interactions.
 * Uses kotlinx-coroutines-test to test suspend functions.
 *
...
class EventRepositoryFirestoreTest {
    ...
    fun getEvent_success_returnsMappedEvent() = runTest {
        val e = Event(
            uid = "", title = "Title", url = "u", description = "d",
            date = Timestamp.now(),
            location = Location("Zurich", 8.5417, 47.3769),
            tags = listOf("music"), public = true,
            ownerId = "owner", imageUrl = null, capacity = 50, attendeeCount = 0
        )
    ...
    fun getEvent_notFound_throws() = runTest {
        val snap: DocumentSnapshot = mock()
        whenever(snap.id).thenReturn("missing")
    ...
    fun getAllEvents_mapsValidDocs_andSkipsBadOnes() = runTest {
        val valid = Event(
            uid = "", title = "Ok", url = "u", description = "d",
            date = Timestamp.now(),
            location = Location("Zurich", 8.54, 47.37),
            tags = listOf("tag"), public = true,
            ownerId = "o", imageUrl = null, capacity = 10, attendeeCount = 1
        )
        val goodDoc = doc("GOOD", valid)

        // Simulate a doc that throws during toObject -> documentToEvent returns null
        val badDoc = mock<DocumentSnapshot>().also {
    ...
    fun getEventsByTitle_filtersClientSide_caseInsensitive_trim() = runTest {
        val e1 = Event("", " Rock Night ", "u", "d", Timestamp.now(),
            Location("ZRH", 0.0, 0.0), listOf("m"), true, "a", null, 1, 0)
        val e2 = Event("", "rock night", "u", "d", Timestamp.now(),
            Location("ZRH", 0.0, 0.0), listOf("m"), true, "b", null, 1, 0)
        val e3 = Event("", "Jazz", "u", "d", Timestamp.now(),
            Location("ZRH", 0.0, 0.0), listOf("m"), true, "c", null, 1, 0)
    ...
    fun addEvent_withBlankUid_generatesAndPersists_withFilledUid() = runTest {
        ...
        val input = Event(
            uid = "", title = "T", url = "u", description = "d",
            date = Timestamp.now(),
            location = Location("L", 0.0, 0.0),
            tags = emptyList(), public = true, ownerId = "o",
            imageUrl = null, capacity = 1, attendeeCount = 0
        )
    ...
    fun editEvent_setsNewValueOnDocument_withSameId() = runTest {
        ...
        val updated = Event(
            uid = "E1", title = "Updated", url = "u", description = "d",
            date = Timestamp.now(),
            location = Location("L", 0.0, 0.0),
            tags = listOf("x"), public = false, ownerId = "o",
            imageUrl = null, capacity = 5, attendeeCount = 2
        )
