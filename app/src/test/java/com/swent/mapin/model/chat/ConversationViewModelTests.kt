...
class ConversationViewModelTest {
  ...
  //Assisted by GPT
  @Test
  fun `observeConversationsForCurrentUser overrides name and profile picture for 1-to-1 chat`() = runTest {
        // Mock FirebaseAuth
        ...
        whenever(query.orderBy("lastMessageTimestamp", Query.Direction.DESCENDING)).thenReturn(query)
        ...
        val conversation = Conversation(
            ...
            participants = listOf(
                UserProfile(userId = "user123", name = "Current User", bio = "", hobbies = emptyList(), location = ""),
                UserProfile(userId = "user456", name = "Other User", bio = "", hobbies = emptyList(), location = "")
            ),
        ...
        whenever(query.addSnapshotListener(any())).thenAnswer { invocation ->
            val listener = invocation.getArgument<com.google.firebase.firestore.EventListener<com.google.firebase.firestore.QuerySnapshot>>(0)
        ...
        Assert.assertNotNull("Flow should have emitted at least one conversation", observedConversation)
