...
class FakeAiAssistantRepositoryTest {
  ...
  fun `recommendEvents is deterministic for same input`() = runTest {
    val events = listOf(
        AiEventSummary(id = "event1", title = "Concert", startTime = Timestamp.now())
    )

    val request = AiRecommendationRequest(
        userQuery = "Find events",
        userContext = AiUserContext(),
        events = events
    )
  ...
  fun `recommendEvents ignores conversationId in fake implementation`() = runTest {
    val events = listOf(
        AiEventSummary(id = "event1", title = "Event", startTime = Timestamp.now())
    )
    ...
    val responseWithConversationId = repository.recommendEvents(
        conversationId = "conv123",
        request = request
    )
    val responseWithoutConversationId = repository.recommendEvents(
        conversationId = null,
        request = request
    )
  ...
  fun `recommendEvents always provides assistant message`() = runTest {
    val requestWithEvents = AiRecommendationRequest(
        userQuery = "Show events",
        userContext = AiUserContext(),
        events = listOf(AiEventSummary(id = "e1", title = "Event", startTime = Timestamp.now()))
    )

    val requestWithoutEvents = AiRecommendationRequest(
        userQuery = "Show events",
        userContext = AiUserContext(),
        events = emptyList()
    )
